...
# Language-specific messages (English subset for brevity; full Amharic omitted to fit limit)
MESSAGES = {
    "en": {
    ...
    "am": {  # Placeholder; full Amharic in Part 2 to avoid exceeding limit
        "welcome": "እንኳን ወደ ቤኑ ስታርትአፕ ድጋፍ ቦት በደህና መጡ!\nለመመዝገብ ቋንቋዎን ይምረጡ:"
        # Remaining keys omitted here; included in Part 2
    }
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not username:
        await update.message.reply_text("🌟 *Please set a Telegram username to use this bot.* 🌟", parse_mode="Markdown")
        ...
        await update.message.reply_text(f"🌟 *{messages['welcome']}* 🌟", reply_markup=reply_markup, parse_mode="Markdown")
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not username:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(f"🌟 *{messages['options']}* 🌟", reply_markup=reply_markup, parse_mode="Markdown")
    elif status == "Denied":
        await update.message.reply_text(f"🌟 *{messages['denied_message']}* 🌟", parse_mode="Markdown")
    elif status == "Pending":
        await update.message.reply_text(f"🌟 *{messages['pending_message']}* 🌟", parse_mode="Markdown")
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text(f"🌟 *{messages['options']}* 🌟", reply_markup=reply_markup, parse_mode="Markdown")
...
async def resources(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for training in PAST_TRAININGS:
        ...
        section = f"✨ *{training['name']}* _({training['date']})_\n{' | '.join(links)}\n_{training['description']}_"
    ...
    keyboard = [[InlineKeyboardButton("🎥 Videos Only", callback_data="filter:videos"), InlineKeyboardButton("📜 Docs Only", callback_data="filter:resources")],
    ...
    await query.message.reply_text(formatted_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard), disable_web_page_preview=True)

async def training_events(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = context.user_data.get("lang", "en")
    query = update.callback_query
    messages = MESSAGES[lang]
    past_sections = [f"🌟 *{t['name']}* _({t['date']})_\n_{t['description']}_" for t in PAST_TRAININGS]
    ...
    keyboard = [[InlineKeyboardButton("📚 Resources", callback_data="cmd:resources"), InlineKeyboardButton("✍️ Sign Up", callback_data="cmd:signup")],
...
async def networking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    messages = MESSAGES[lang]
    network_companies = {"Biscuit Production": [{"name": "EthioBiscuit Co.", "description": "Produces fortified biscuits", "contact": "+251912345678"},
                                                {"name": "Benu Biscuits", "description": "Specializes in local biscuit varieties", "contact": "Private"}],
                         "Agriculture": [{"name": "AgriGrow Ethiopia", "description": "Supplies wheat and grains", "contact": "+251987654321"},
...
async def news(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    messages = MESSAGES[lang]
    news_items = ["🌟 *March 12, 2025*: _Benu secured ETB 2.9M from SWR Ethiopia._", "🌟 *April 10, 2025*: _First training held—29 saleswomen trained! See /training_events._",
                  "🌟 *May 2025*: _New production line launches._", "🌟 *May 15, 2025*: _Networking Event—register at /networking or /training_events._"]
...
async def update_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    messages = MESSAGES[lang]
    keyboard = [[InlineKeyboardButton("Name", callback_data="profile:name"), InlineKeyboardButton("Phone", callback_data="profile:phone")],
...
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "asking" in context.user_data:
        ...
        if step == "name":
            ...
            context.user_data["signup_step"] = "description"
            ...
            data = [username, context.user_data["name"], context.user_data["phone"], context.user_data["email"], context.user_data["company"], text, str(chat_id), datetime.now().isoformat(), "Pending"]
            users_sheet.append_row(data)
            await context.bot.send_message(MANAGER_CHAT_ID, f"New Registration:\nUsername: {username}\nName: {data[1]}\nPhone: {data[2]}\nEmail: {data[3]}\nCompany: {data[4]}\nDescription: {data[5]}",
                                           reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Approve", callback_data=f"approve:{username}"),
                                                                               InlineKeyboardButton("Deny", callback_data=f"deny:{username}")]]))
        ...
        if step == "company":
            ...
            keyboard = [[InlineKeyboardButton("Biscuit Production", callback_data="cat:Biscuit Production"), InlineKeyboardButton("Agriculture", callback_data="cat:Agriculture")],
                        [InlineKeyboardButton("Packaging", callback_data="cat:Packaging"), InlineKeyboardButton("Marketing", callback_data="cat:Marketing")],
                        [InlineKeyboardButton("Done", callback_data="cat:done")], [InlineKeyboardButton("🔙 Back to Main Menu", callback_data="cmd:main_menu")]]
            ...
            data = [str(chat_id), context.user_data["company"], context.user_data["phone"], context.user_data["email"], context.user_data["description"], context.user_data["manager"],
                    ",".join(context.user_data.get("categories", [])), datetime.now().isoformat(), "Yes" if context.user_data["public"] else "No"]
        ...
        if step < len(module["quiz"]):
            ...
            keyboard.append([InlineKeyboardButton("🔙 Back to Main Menu", callback_data="cmd:main_menu")])
            await update.message.reply_text(f"🌟 *{MESSAGES[lang]['quiz_question'].format(num=step + 1, q=next_q['q'])}* 🌟", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        else:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if "lang:" in query.data:
            ...
            await query.edit_message_text(f"🌟 *{MESSAGES[lang_choice]['signup_prompt']}* 🌟", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        elif "cmd:" in query.data:
            cmd = query.data.split("cmd:")[1]
            handlers = {"ask": ask, "resources": resources, "training_events": training_events, "networking": networking, "news": news, "contact": contact,
                        "subscribenews": subscribenews, "learn_startup_skills": learn_startup_skills, "update_profile": update_profile, "ask_again": ask,
                        "main_menu": lambda u, c: show_options(u, c, lang), "all_resources": lambda u, c: all_resources(u, c, lang), "signup": signup, "register": register,
                        "cancel": lambda u, c: update.message.reply_text("Registration cancelled.", parse_mode="Markdown")}
            if cmd in handlers:
                await handlers[cmd](update, context)
        elif "approve:" in query.data:
            username = query.data.split("approve:")[1]
            cell = users_sheet.find(username)
            if cell and query.from_user.id == int(MANAGER_CHAT_ID):
                users_sheet.update_cell(cell.row, 9, "Approved")
                chat_id = users_sheet.cell(cell.row, 7).value
                await context.bot.send_message(chat_id, f"🌟 *{MESSAGES[lang]['approved_message']}* 🌟", parse_mode="Markdown")
                await query.edit_message_text(f"Approved {username}.")
            ...
            if cell and query.from_user.id == int(MANAGER_CHAT_ID):
                ...
                chat_id = users_sheet.cell(cell.row, 7).value
                await context.bot.send_message(chat_id, f"🌟 *{MESSAGES[lang]['denied_message']}* 🌟", parse_mode="Markdown")
                await query.edit_message_text(f"Denied {username}.")
            ...
            formatted_text = f"🌟 *{MESSAGES[lang]['resources_title']}* 🌟\n\n" + "\n🌟====🌟\n".join(sections)
            keyboard = [[InlineKeyboardButton("🎥 Videos Only", callback_data="filter:videos"), InlineKeyboardButton("📜 Docs Only", callback_data="filter:resources")],
                        [InlineKeyboardButton("⬇️ Get All Resources", callback_data="cmd:all_resources"), InlineKeyboardButton("🔙 Back to Main Menu", callback_data="cmd:main_menu")]]
            ...
            if all(prereq in completed for prereq in module["prereq"]):
                ...
                keyboard.append([InlineKeyboardButton("🔙 Back to Main Menu", callback_data="cmd:main_menu")])
                await query.message.reply_text(f"🌟 *{MESSAGES[lang]['quiz_start'].format(name=module['name'])}* 🌟\n🌟 *{MESSAGES[lang]['quiz_question'].format(num=1, q=module['quiz'][0]['q'])}* 🌟",
            ...
            if step < len(module["quiz"]):
                ...
                keyboard = [[InlineKeyboardButton(opt, callback_data=f"quiz:{opt}")] for opt in next_q["options"]]
                keyboard.append([InlineKeyboardButton("🔙 Back to Main Menu", callback_data="cmd:main_menu")])
                await query.message.reply_text(f"🌟 *{MESSAGES[lang]['quiz_question'].format(num=step + 1, q=next_q['q'])}* 🌟", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
            else:
                score = context.user_data.get("quiz_score", 0)
                context.user_data["completed_modules"] = context.user_data.get("completed_modules", []) + [module_id]
...
def main():
    ...
    app.run_webhook(listen="0.0.0.0", port=port, url_path="/", webhook_url=f"https://{os.environ['RENDER_EXTERNAL_HOSTNAME']}/")
    print("Bot is running on Render...")

if __name__ == "__main__":
    main()

# Full Amharic messages (omitted from Part 1 for space)
MESSAGES["am"] = {
    "welcome": "እንኳን ወደ ቤኑ ስታርትአፕ ድጋፍ ቦት በደህና መጡ!\nለመመዝገብ ቋንቋዎን ይምረጡ:",
    "options": "አማራጭ ይምረጡ:",
    "ask": "ጥያቄ ይጠይቁ",
